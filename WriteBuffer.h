...
class NVM_WRITE_BUFFER {
    ...
    // scheduling mode: this can be used to optimize writing to NVMs, cache lines within the same rowbuffer are
    // prioritized to be written together
    int sched_mode;

    // flush threshold percentage; this determines at what percentage of max number of entries triggers flushing the
    // write buffer: e.g., 50% means when we have 50% of the size entries, we should attempt flushing
    int flush_th;

    // This indicates the low threshold, where the controller will bring down the number of entries to, whenever it
    ...
    int entry_size; // this determines the granularity of the write requests, ideally this should be similar to cache
                    // line size

    bool
        still_flushing; // This indicates that the controller is still trying to bring down the entries to low threshold

  public:
    // Constructor
    NVM_WRITE_BUFFER(int Size, int Sched_mode, int Entry_size, int Flush_th, int low_th) {
        flush_th_low = low_th;
        max_size = Size;
    ...
    // This enables searching if a request exists on the write buffer (this is important for correctness and not to
    // break memory consistency)
