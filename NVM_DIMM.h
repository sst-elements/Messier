...
// This class structure represents NVM-Based DIMM, including the NVM-DIMM
// controller
class NVM_DIMM : public ComponentExtension {
  ...
  // This is the requests buffer, where all transactions are buffered before
  ...
  // This is used to quickly track the number of writes complete at a specific
  // cycle to remove them from the currently executed writes
  std::map<long long int, int> WRITES_COMPLETE;

  // This is used to quickly track the number of reads complete at a specific
  // cycle to remove them from the currently executed reads
  ...
  // This holds a pointer to the ranks. Note: for NVM devices, most likely there
  // will be no ranks concept, thus it will be only one rank
  RANK **ranks;
  ...
  // This structure prevents returning data before checking the cache, to avoid
...
public:
  ...
  // SecondChance: This is for evaluating the idea of issuing requests to free
  // banks
  BANK *getFreeBank(long long int add);

  // This determines the location of the block (in which rank), based on the
  // interleaving policy
  int WhichRank(long long int add);

  // This determines the location of the block (in which bank), based on the
  // interleaving policy
  int WhichBank(long long int add);

  // bool push_request(NVM_Request * req) { if(transactions.size() >=
  // params->max_requests) return false; else {transactions.push_back(req);
  // return true; }}

  bool push_request(NVM_Request *req) {
    transactions.push_back(req);
    if (req->Read)
      TIME_STAMP[req] = cycles;
    return true;
  }

  // This is the optimized version that basiclly tries to find out if there is
  // any possibility to achieve a row buffer hit from the current transactions
