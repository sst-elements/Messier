...
// Finding an entry request, mainly to check if the request exists on write-buffer before proceeding to submit the
...
// Popping up the first entry in the write buffer, this is called by the NVM memory controller when it is idle or the
// flush signal is triggered in the write buffer
...
// Towards out-of-order execution of writes. With this we enable the memory controller to execute and erase any write
// request
void NVM_WRITE_BUFFER::erase_entry(NVM_Request *TEMP) {
    ...
    if (mem_reqs.size() != curr_entries)
        std::cout << "Massive error 3  curr_entrie=" << curr_entries << "  mem_reqs.size()= " << mem_reqs.size()
                  << std::endl;
