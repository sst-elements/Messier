...
class NVM_PARAMS {
  public:
    ...
    // The number of possible outstanding requests inside the internal controller (not that once writes are written to
    // the write buffer, they are deleted)
    int max_outstanding;

    // For power budget (at any time, the controller enforces write_weight*num_current_writes +
    // read_weight*num_current_reads less than or equal max_current_weight
    ...
    // The latency of writing a column. Note that we assume internal PCMs write it directly to the PCM cells and update
    // the row buffer (in case it was row-buffer hit)
    int tCL_W;
