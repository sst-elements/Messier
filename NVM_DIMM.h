...
class NVM_DIMM : public ComponentExtension {
    ...
    // This is used to quickly track the number of writes complete at a specific cycle to remove them from the currently
    // executed writes
    std::map<long long int, int> WRITES_COMPLETE;

    // This is used to quickly track the number of reads complete at a specific cycle to remove them from the currently
    // executed reads
    ...
    // This holds a pointer to the ranks. Note: for NVM devices, most likely there will be no ranks concept, thus it
    // will be only one rank
    RANK **ranks;
  ...
  public:
    ...
    // bool push_request(NVM_Request * req) { if(transactions.size() >= params->max_requests) return false; else
    // {transactions.push_back(req); return true; }}
    ...
    // This is the optimized version that basiclly tries to find out if there is any possibility to achieve a row buffer
    // hit from the current transactions
