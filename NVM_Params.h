...
public:
  ...
  // The number of possible outstanding requests inside the internal controller
  // (not that once writes are written to the write buffer, they are deleted)
  int max_outstanding;

  // For power budget (at any time, the controller enforces
  // write_weight*num_current_writes + read_weight*num_current_reads less than
  // or equal max_current_weight
  ...
  // The clock of the IO bus, this typically should be similar to the internal
  // controller clock
  float io_clock;

  // The time to send a command from the internal controller till buffered
  // inside the NVM chips
  int tCMD;

  // This is the access latency of a column for an already activated row buffer
  int tCL;

  // This is the actual read latency of a row and installing it on the row
  // buffer
  int tRCD;

  // The latency of writing a column. Note that we assume internal PCMs write it
  // directly to the PCM cells and update the row buffer (in case it was
  // row-buffer hit)
  int tCL_W;
  ...
  // This determines the number of devices on the DIMM Ranks (the DIMM bus width
  // is num_devices X  device_width)
  int num_devices;
  ...
  // This indicates the maximum number of conccurent writes to NVM chips (not
  // including those on the write buffer)
  int max_writes;
  ...
  // This inidicates the size of the group of banks to be locked when writing in
  // case of adaptive writes
  int group_size;
  ...
  // This indicates the unit of module, if N, this means for each N reads, we
  // service one write
  int modulo_unit;
