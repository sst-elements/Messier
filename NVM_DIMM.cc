...
NVM_DIMM::NVM_DIMM(ComponentId_t id, NVM_PARAMS par) : ComponentExtension(id) {
  ...
  WB = new NVM_WRITE_BUFFER(params->write_buffer_size, 0,
                            64 /*write buffer granularity, now assume 64B */,
                            params->flush_th, params->flush_th_low);

  if (params->cache_enabled)
    cache = new NVM_CACHE(params->cache_size, params->cache_assoc,
                          params->cache_latency, params->cache_bs);
  else
    cache = NULL;
...
// To maximize performance, we will assume that consecutive row numbers be
// located in different ranks
...
bool NVM_DIMM::tick() {
  ...
  // We start with checking if any read request is ready at NVM, to schdule
  // reading it form the NVM Chip
  schedule_delivery();

  if (params->modulo) {

    if (!WB->empty()) {
      // Here we should check if the write buffer is full or exceeds the
      // threshold and try to flush at least a request
...
void NVM_DIMM::schedule_delivery() {
  ...
  while (st_1 != en_1) {
    ...
    if (st_1->second <= cycles) {
      ...
      if (ready) // This means that the request is ready and the data is ready
                 // to be ready by internal controller
      {

        // Occuping the rank and back for reading the ready data
        getRank(add)->setBusyUntil(cycles + params->tCMD + params->tCL +
                                   params->tBURST);
        (getBank(add))
            ->setBusyUntil(cycles + params->tCMD + params->tCL +
                           params->tBURST);
        (getBank(add))->set_last(true);
        ready_trans[st_1->first] =
            cycles + params->tCMD + params->tCL + params->tBURST;
        (st_1->first)->meta_data = EventType::READ_COMPLETION;
        m_EventChan->send(
            params->tCMD + params->tCL + params->tBURST,
            new MessierEvent(st_1->first, EventType::READ_COMPLETION));
...
bool NVM_DIMM::try_flush_wb() {
  ...
  if (flush_write) {
    ...
    while (st_wl != en_wl) {
      ...
      if ((!params->adaptive_writes ||
           (group_locked == (WhichBank(temp->Address) / params->group_size))) &&
          ready && (MAX_WRITES > curr_writes) &&
          ((params->write_weight * curr_writes +
            params->read_weight * curr_reads) <=
           (params->max_current_weight - params->write_weight))) {

        removed = true;
        WB->erase_entry(temp);
        // Note that the rank will be busy for the time of sending the data to
        // the bank, in addition to sending the command
        getRank(add)->setBusyUntil(cycles + params->tCMD + params->tBURST);
        (temp_bank)->setBusyUntil(cycles + params->tCMD + params->tCL_W +
                                  params->tBURST);
        temp_bank->set_last(false); // setting it to write
        temp_bank->set_last_address(temp->Address);
        curr_writes++;
        WRITES_COMPLETE[cycles + params->tCMD + params->tCL_W +
                        params->tBURST]++;
...
bool NVM_DIMM::find_in_wb(NVM_Request *temp) {
  ...
  if (WB->find_entry(temp->Address) != NULL) {
    removed = true;
    MemRespEvent *respEvent =
        new MemRespEvent(NVM_EVENT_MAP[temp->req_ID]->getReqId(),
                         NVM_EVENT_MAP[temp->req_ID]->getAddr(),
                         NVM_EVENT_MAP[temp->req_ID]->getFlags());
...
bool NVM_DIMM::row_buffer_hit(long long int add, long long int bank_add) {
  ...
  if (cacheline_interleave) {
    if ((add / (params->num_banks * params->row_buffer_size)) ==
        bank_add / params->num_banks)
      return true;
    else
      return false;

  } else // if bank interleaving (meaning each page goes to a specific bank),
         // consecutive pages go to different banks
  {
...
bool NVM_DIMM::pop_optimal() {
  ...
  while (st != en) {
    ...
    if ((!params->adaptive_writes ||
         group_locked != (WhichBank(temp->Address) / params->group_size)) &&
        (HOLD.find(temp->req_ID) == HOLD.end()) && temp->Read &&
        (corresp_rank->getBusyUntil() < cycles) &&
        (corresp_bank->getBusyUntil() < cycles) && !corresp_bank->getLocked() &&
        (outstanding.size() < params->max_outstanding)) {

      if (row_buffer_hit(temp->Address, corresp_bank->getRB())) {
        time_ready = cycles + 1;
        outstanding.push_back(temp);
        transactions.erase(st);
        // Lock the bank so no other request comes in and try to activate
        // another row while waiting for the activation

        corresp_bank->setLocked(true, cycles);
        temp->meta_data = EventType::DEVICE_READY;
        m_EventChan->send(time_ready - cycles,
                          new MessierEvent(temp, EventType::DEVICE_READY));
        return true;
      }
    }
  ...
  if (found) {
    ...
    while (st != en) {
      ...
      } else if (temp->Read) // if read request
      {
        ...
        {
          // First find out the corresponding bank to the read request and check
          // if busy
          RANK *corresp_rank = getRank(temp->Address);
          BANK *corresp_bank = getBank(temp->Address);
          ...
          if ((!params->adaptive_writes ||
               group_locked !=
                   (WhichBank(temp->Address) / params->group_size)) &&
              (HOLD.find(temp->req_ID) == HOLD.end()) &&
              (corresp_rank->getBusyUntil() < cycles) &&
              (((corresp_bank->getBusyUntil() < cycles) &&
                !corresp_bank->getLocked()) ||
               (params->write_cancel && !WB->flush() && !corresp_bank->read() &&
                (corresp_bank->getBusyUntil() - cycles <
                 (100 - 4 * WB->getSize()) * 1.0 * params->tCL_W / 100.0))) &&
              (outstanding.size() < params->max_outstanding)) {

            // If this comes here due to write cancellation: do the right
            // business
            if (params->write_cancel &&
                (corresp_bank->getBusyUntil() >= cycles) &&
                !corresp_bank->read() && !WB->flush() &&
                (corresp_bank->getBusyUntil() - cycles <
                 (100 - 4 * WB->getSize()) * 1.0 * params->tCL_W / 100.0)) {
            ...
            if (row_buffer_hit(temp->Address, corresp_bank->getRB())) {
              time_ready = cycles + 1;
              issued = true;
            } else if ((params->write_weight * curr_writes +
                        params->read_weight * curr_reads) <=
                       (params->max_current_weight - params->read_weight)) {
      ...
      if (params->cache_persistent) {
        ...
        if (dirty) {
          ...
          if (!WB->full()) {
            ...
            m_EventChan->send(
                50, e); // Try after 50 cycles, to see if we got room in the
                        // write buffer to evict the dirty block
            return;     //
          }
        } else {
          MemRespEvent *respEvent =
...
void NVM_DIMM::handleRequest(SST::Event *e) {
  ...
  MessierComponent::MemReqEvent *event =
      dynamic_cast<MessierComponent::MemReqEvent *>(e);
  ...
  if (cache != NULL) {
    ...
    if (!tmp2->Read) {
      ...
      tmp2->meta_data = params->cache_persistent ? EventType::HIT_MISS
                                                 : EventType::INVALIDATE_WRITE;

      MessierEvent *mess = new MessierEvent(
          tmp2, params->cache_persistent ? EventType::HIT_MISS
                                         : EventType::INVALIDATE_WRITE);
      m_EventChan->send(params->cache_persistent ? params->cache_latency : 1,
                        mess);
      ...
      m_EventChan->send(params->cache_latency,
                        new MessierEvent(tmp2, EventType::HIT_MISS));
    }
  }
