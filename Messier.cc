...
// Here we do the initialization of the Samba units of the system, connecting them to the cores and instantiating TLB
// hierachy objects for each one
...
Messier::Messier(SST::ComponentId_t id, SST::Params &params) : Component(id) {
    ...
    // m_memChan = configureLink(link_buffer, "0ps", new Event::Handler<TLBhierarchy>(TLB[i],
    // &TLBhierarchy::handleEvent_CPU));
