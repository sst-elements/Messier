...
NVM_DIMM::NVM_DIMM(ComponentId_t id, NVM_PARAMS par) : ComponentExtension(id) {
    ...
    WB = new NVM_WRITE_BUFFER(params->write_buffer_size, 0,
                              64 /*write buffer granularity, now assume 64B */, params->flush_th,
                              params->flush_th_low);


    if (params->cache_enabled)
        cache = new NVM_CACHE(params->cache_size, params->cache_assoc, params->cache_latency,
                              params->cache_bs);
    else
        cache = NULL;
    ...
    if (flush_write) {
        ...
        while (st_wl != en_wl) {
            ...
            if ((!params->adaptive_writes ||
                 (group_locked == (WhichBank(temp->Address) / params->group_size))) && ready &&
                (MAX_WRITES > curr_writes) &&
...
bool NVM_DIMM::find_in_wb(NVM_Request *temp) {
    ...
    if (WB->find_entry(temp->Address) != NULL) {
        ...
        MemRespEvent *respEvent = new MemRespEvent(
            NVM_EVENT_MAP[temp->req_ID]->getReqId(), NVM_EVENT_MAP[temp->req_ID]->getAddr(),
...
bool NVM_DIMM::pop_optimal() {
    ...
    while (st != en) {
        ...
        if ((!params->adaptive_writes ||
            ...
            (corresp_rank->getBusyUntil() < cycles) && (corresp_bank->getBusyUntil() < cycles) &&
            !corresp_bank->getLocked() && (outstanding.size() < params->max_outstanding)) {
        ...
        while (st != en) {
            ...
            if ((!temp->Read)) {
                if (!WB->full()) {
                    ...
                    MemRespEvent *respEvent = new MemRespEvent(
                        NVM_EVENT_MAP[temp->req_ID]->getReqId(),
            ...
            } else if (temp->Read)// if read request
            {
                ...
                } else //if(!removed)
                {
                    ...
                    if ((!params->adaptive_writes ||
                        ...
                        if (params->write_cancel && (corresp_bank->getBusyUntil() >= cycles) &&
            ...
            if (params->cache_persistent) {
                ...
                if (dirty) {
                    ...
                    if (!WB->full()) {
                        ...
                        m_EventChan->send(50,
                                          e); // Try after 50 cycles, to see if we got room in the write buffer to evict the dirty block
                        return; //
                    }
                } else {
                    MemRespEvent *respEvent = new MemRespEvent(
...
void NVM_DIMM::handleRequest(SST::Event *e) {
    ...
    if (cache != NULL) {
        ...
        if (!tmp2->Read) {
            ...
            MessierEvent *mess = new MessierEvent(tmp2,
                                                  params->cache_persistent ? EventType::HIT_MISS
                                                                           : EventType::INVALIDATE_WRITE);
