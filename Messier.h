...
class Messier : public SST::Component {
  public:
    SST_ELI_REGISTER_COMPONENT(Messier, "Messier", "Messier", SST_ELI_ELEMENT_VERSION(1, 0, 0), "Messier Component",
                               COMPONENT_CATEGORY_MEMORY)

    SST_ELI_DOCUMENT_PARAMS(
        {"clock", "Internal Controller Clock Rate.", "1.0 Ghz"}, {"memory_clock", "NVM Chips Clock Rate.", "1.0 Ghz"},
        ...
        {"max_outstanding", "This determines the max number of outstanding/executing requests inside the NVMDIMM",
         "16"},
        ...
        {"num_devices", "The number of NVM-Chip devices", "8"}, {"num_ranks", "The number of ranks", "1"},
        ...
        {"cache_latency", "The cache latency", "0"}, {"cache_size", "The cache size in KBs", "0"},
        {"cache_bs", "The cache block size", "0"}, {"cache_assoc", "The cache associativity", "0"},
        {"row_buffer_size", "The row buffer size in bytes", "8192"},
        {"flush_th",
         "This determines the percentage of max number of entries in write buffer that triggers flushing it", "50"},
        {"flush_th_low", "This determines the percentage of the low threshold number of entries", "30"},
        {"max_writes",
         "This determines the maximum number of concurrent writes at NVM chips (not including those on buffer", "1"},
        {"cacheline_interleaving", "This determines if cacheline interleaving is used (1) or bank interleaving (0) ",
         "1"},
    ...
    SST_ELI_DOCUMENT_STATISTICS({"reads", "Determine the number of reads", "reads", 1},
                                {"writes", "Determine the number of writes", "writes", 1},
                                {"avg_time", "The average time spent on each read request", "cycles", 3},
                                {"histogram_idle", "The histogram of cycles length while controller is idle", "cycles",
                                 1})
  ...
  private:
    ...
    Messier(const Messier &) = delete;        // do not implement
    void operator=(const Messier &) = delete; // do not implement
