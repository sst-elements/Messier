...
class BANK {
    ...
    // This means the bank is locked until serving a specific request, this is important to avoid races in the time
    // between activation ready and the controller read the data, the block must remain blocked until the data is read,
    // and void being stolen by another request
    bool locked;
