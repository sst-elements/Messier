...
#define MESSIER_VERBOSE(LEVEL, OUTPUT)                                         \
  if (verbosity >= (LEVEL))                                                    \
  OUTPUT
...
void Messier::parser(NVM_PARAMS *nvm, SST::Params &params) {
  ...
  nvm->size =
      (uint32_t)params.find<uint32_t>("size", 8388608); // in KB, which mean 8GB

  nvm->write_buffer_size =
      (uint32_t)params.find<uint32_t>("write_buffer_size", 128);
  ;

  nvm->max_outstanding = (uint32_t)params.find<uint32_t>("max_outstanding", 16);

  nvm->max_current_weight =
      (uint32_t)params.find<uint32_t>("max_current_weight", 120);
  ;
  ...
  int cache_interleave =
      (uint32_t)params.find<uint32_t>("cacheline_interleaving", 1);
  ...
  nvm->row_buffer_size =
      (uint32_t)params.find<uint32_t>("row_buffer_size", 8192);
  ...
  nvm->group_size =
      (uint32_t)params.find<uint32_t>("group_size", nvm->num_banks);

  nvm->lock_period = (uint32_t)params.find<uint32_t>("lock_period", 10000);
}

// Here we do the initialization of the Samba units of the system, connecting
// them to the cores and instantiating TLB hierachy objects for each one
...
Messier::Messier(SST::ComponentId_t id, SST::Params &params) : Component(id) {
  ...
  // m_memChan = configureLink(link_buffer, "0ps", new
  // Event::Handler<TLBhierarchy>(TLB[i], &TLBhierarchy::handleEvent_CPU));

  nvm_params = new NVM_PARAMS();

  // This converts the sst paramters into an object of NVM_PARAMS, used to
  // instantiate NVM_DIMM
  ...
  m_memChan = configureLink(
      link_buffer, "1ns",
      new Event::Handler<NVM_DIMM>(DIMM, &NVM_DIMM::handleRequest));

  sprintf(link_buffer, "event_bus");

  event_link = configureSelfLink(
      link_buffer, "1ns",
      new Event::Handler<NVM_DIMM>(DIMM, &NVM_DIMM::handleEvent));
