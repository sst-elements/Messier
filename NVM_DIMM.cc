...
NVM_DIMM::NVM_DIMM(ComponentId_t id, NVM_PARAMS par) : ComponentExtension(id) {
    ...
    WB = new NVM_WRITE_BUFFER(params->write_buffer_size, 0, 64 /*write buffer granularity, now assume 64B */,
...
int NVM_DIMM::WhichRank(long long int add) { return (add / params->row_buffer_size) % (params->num_ranks); }
...
bool NVM_DIMM::tick() {
    ...
    if (params->modulo) {
        ...
        if (!WB->empty()) {
            // Here we should check if the write buffer is full or exceeds the threshold and try to flush at least a
            // request
    ...
    while (st_1 != en_1) {
        ...
        if (st_1->second <= cycles) {
            ...
            if (ready) // This means that the request is ready and the data is ready to be ready by internal controller
            {
                ...
                m_EventChan->send(params->tCMD + params->tCL + params->tBURST,
...
bool NVM_DIMM::try_flush_wb() {
    ...
    if (flush_write) {
        ...
        while (st_wl != en_wl) {
            ...
            if ((!params->adaptive_writes || (group_locked == (WhichBank(temp->Address) / params->group_size))) &&
                ready && (MAX_WRITES > curr_writes) &&
                ((params->write_weight * curr_writes + params->read_weight * curr_reads) <=
                 (params->max_current_weight - params->write_weight))) {

                removed = true;
                WB->erase_entry(temp);
                // Note that the rank will be busy for the time of sending the data to the bank, in addition to sending
                // the command
...
bool NVM_DIMM::find_in_wb(NVM_Request *temp) {
    ...
    if (WB->find_entry(temp->Address) != nullptr) {
        removed = true;
        auto *respEvent =
            new MemRespEvent(NVM_EVENT_MAP[temp->req_ID]->getReqId(), NVM_EVENT_MAP[temp->req_ID]->getAddr(),
...
bool NVM_DIMM::pop_optimal() {
    ...
    while (st != en) {
        ...
        if ((!params->adaptive_writes || group_locked != (WhichBank(temp->Address) / params->group_size)) &&
            (HOLD.find(temp->req_ID) == HOLD.end()) && temp->Read && (corresp_rank->getBusyUntil() < cycles) &&
            ...
            if (row_buffer_hit(temp->Address, corresp_bank->getRB())) {
                ...
                transactions.erase(st);
                // Lock the bank so no other request comes in and try to activate another row while waiting for the
                // activation
        ...
        while (st != en) {
            ...
            if ((!temp->Read)) {
                if (!WB->full()) {
                    ...
                    auto *write_req = new NVM_Request();
                    write_req->req_ID = 0;
                    ...
                    auto *respEvent = new MemRespEvent(NVM_EVENT_MAP[temp->req_ID]->getReqId(),
            ...
            } else if (temp->Read) // if read request
            {
                ...
                } else // if(!removed)
                {
                    ...
                    if ((!params->adaptive_writes || group_locked != (WhichBank(temp->Address) / params->group_size)) &&
                        (HOLD.find(temp->req_ID) == HOLD.end()) && (corresp_rank->getBusyUntil() < cycles) &&
                        (((corresp_bank->getBusyUntil() < cycles) && !corresp_bank->getLocked()) ||
                        ...
                        if (params->write_cancel && (corresp_bank->getBusyUntil() >= cycles) && !corresp_bank->read() &&
                            !WB->flush() &&
                            (corresp_bank->getBusyUntil() - cycles <
                             (100 - 4 * WB->getSize()) * 1.0 * params->tCL_W / 100.0)) {
                        ...
                        if (row_buffer_hit(temp->Address, corresp_bank->getRB())) {
                            time_ready = cycles + 1;
                            issued = true;
                        } else if ((params->write_weight * curr_writes + params->read_weight * curr_reads) <=
                        ...
                        if (issued) {
                            ...
                            transactions.erase(st);
                            removed = true;
                            // Lock the bank so no other request comes in and try to activate another row while waiting
                            // for the activation
                            corresp_bank->setLocked(true, cycles);
                            temp->meta_data = EventType::DEVICE_READY;
                            m_EventChan->send(time_ready - cycles, new MessierEvent(temp, EventType::DEVICE_READY));
...
void NVM_DIMM::handleEvent(SST::Event *e) {
    ...
    auto *temp_ptr = dynamic_cast<MessierComponent::MessierEvent *>(e);
            ...
            if (params->cache_persistent) {
                ...
                if (dirty) {
                    ...
                    if (!WB->full()) {
                        ...
                        m_EventChan->send(50, e); // Try after 50 cycles, to see if we got room in the write buffer to
...
void NVM_DIMM::handleRequest(SST::Event *e) {
    ...
    auto *event = dynamic_cast<MessierComponent::MemReqEvent *>(e);

    auto *tmp = new NVM_Request();
    ...
    if (cache != nullptr) {
        ...
        if (!tmp2->Read) {
            ...
            auto *mess =
                new MessierEvent(tmp2, params->cache_persistent ? EventType::HIT_MISS : EventType::INVALIDATE_WRITE);
    ...
    if (!(params->cache_persistent && (cache != nullptr) && (!tmp->Read)))
